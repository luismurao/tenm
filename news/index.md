...
- Time-specific niche modeling (TENM) is a novel approach that allows
  calibrating niche models with high temporal resolution spatial
  ...
  yearly, monthly, or even daily basis. This feature distinguishes it
  from traditional models that rely on averaged temporal data.
  ...
  - Time-specific background generation: generate background points for
    ...
    occurrence and environmental data: The number of background points
    for each year is proportional to the number of occurrences for each
    year of observation.
  ...
  - Time-specific model calibration: calibrate time-specific niche
    models using minimum volume ellipsoids. It fits numerous models
    based on a combination of user-set parameters, including different
